// ─── OSINT Verification Core ─────────────────────────────────────────────────
// Shared module: used by both the OSINT route AND the orchestrator.
// Contains all verification logic (VIES, CF, ANAC, reliability scoring)
// without Express dependencies.
// ────────────────────────────────────────────────────────────────────────────
...
export function validateFiscalCode(cf: string): FiscalCodeResult {
...
export async function checkVies(vatRaw: string): Promise<VIESResult> {
  ...
  try {
    ...
    return {
      valid: null, country_code: countryCode, vat_number: vatNumber,
...
export async function checkAnac(vatNumber: string | undefined, companyName: string | undefined): Promise<ANACResult> {
  ...
  try {
    ...
    return { checked: true, annotations_found: parsed.annotations.length > 0, annotations: parsed.annotations, error: null }
  } catch (err) {
    return {
      checked: false, annotations_found: false, annotations: [],
...
export function computeReliabilityScore(
  ...
  const dimensions: ReliabilityDimensions = {
    legal: 0, contributory: 0, reputation: 0, solidity: 0, consistency: 0,
  }
...
// ── Full OSINT Verification (end-to-end) ────────────────────────────────────

export interface FullOSINTResult {
  vies: VIESResult | null
  fiscal_code: FiscalCodeResult | null
  anac: ANACResult | null
  reliability: { score: number; dimensions: ReliabilityDimensions }
}

export async function runFullOSINT(opts: {
  vat_number?: string
  fiscal_code?: string
  company_name?: string
  counterpart_id?: string
  company_id?: string
}): Promise<FullOSINTResult> {
  const promises: Promise<{ key: string; result: VIESResult | FiscalCodeResult | ANACResult | null }>[] = []

  if (opts.vat_number) {
    promises.push(checkVies(opts.vat_number).then(r => ({ key: 'vies', result: r })))
  } else {
    promises.push(Promise.resolve({ key: 'vies', result: null }))
  }

  if (opts.fiscal_code) {
    promises.push(Promise.resolve({ key: 'fiscal_code', result: validateFiscalCode(opts.fiscal_code) }))
  } else {
    promises.push(Promise.resolve({ key: 'fiscal_code', result: null }))
  }

  if (opts.vat_number || opts.company_name) {
    promises.push(checkAnac(opts.vat_number, opts.company_name).then(r => ({ key: 'anac', result: r })))
  } else {
  ...
  // Persist to counterparts if ID provided
  if (opts.counterpart_id) {
    try {
      const { data: existing } = await supabase
        .from('counterparts')
        .select('verification_json')
        .eq('id', opts.counterpart_id)
        .single()
      ...
      await supabase.from('counterparts').update({
        ...
        osint_verified: true,
        osint_last_check: new Date().toISOString(),
  ...
  return { vies, fiscal_code: cf, anac, reliability: { score, dimensions } }
}
